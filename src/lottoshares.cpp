...
void addShareDrops(CBlock &block){
    ...
    if (myfile.is_open()){
                while ( myfile.good() ){
                    ...
                    boost::split(strs, line, boost::is_any_of(","));
                    if(strs.size()==3){
                        int64 distributionAmount = atoi64(strs[2].c_str());
                        while(distributionAmount>0){
                            dgCount++;
                            sprintf(intStr,"%d",dgCount);
                            CTransaction txNew;
                            ...
                            runningTotalCoins+=txNew.vout[0].nValue;
                            CBitcoinAddress address(convertAddress(strs[1].c_str(),0x30));
                            txNew.vout[0].scriptPubKey.SetDestination( address.Get() );
                            block.vtx.push_back(txNew);
                        }
                    }else{
                        printf("thirtypercent.txt - %s line parse failed\n",line.c_str());
                    }
                }
                myfile.close();
            }else{
    ...
    myfile.open(getShareDropsPath("thirtypercent2.txt").string().c_str());
    if (myfile.is_open()){
                while ( myfile.good() ){
                    std::getline (myfile,line);
                    std::vector<std::string> strs;
                    ...
                    if(strs.size()==3){
                        ...
                        while(distributionAmount>0){
                            ...
                            txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)intStr, (const unsigned char*)intStr + strlen(intStr));
                            txNew.vout.resize(1);
                            if(distributionAmount>1000*COIN){
                        ...
                        printf("thirtypercent2.txt - %s line parse failed\n",line.c_str());
                    }
                }
                myfile.close();
            }else{
                printf("thirtypercent2.txt - required for distribution, not found\n");
            }
